...
export default function Checkin() {
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>
          Intake Form - {leagues.length === 1 ? leagues[0] + " - " : ""}
      ...
      <main className={styles.main}>
        ...
        {submittedAt ? (
          healthAnswers.some((ans) => ans !== false) ? (
          ...
          <>
            {userState.isPending ? (
              ...
              teamsState.isPending ? (
                ...
                <p>
                  Sorry, we didn't find any events coming up in your TeamSnap
                  account. You'll have to fill in the form manually.
                </p>
              ) : (
                <>
                  ...
                  <p>
                    ...
                    information from TeamSnap for that event to fill the first
                    fields of the form.
                  </p>
                  <table className={styles.eventPickerTable}>
                    ...
                    <tbody>
                      {events.map((evt) => {
                        const team = activeTeams.find(
                          (t) => t.id === evt.teamId
                        );
                        const onClickEvent: ChangeEventHandler<HTMLInputElement> = () => {
                          setEventId(evt.id);
                          ...
                          loadTeamContactsForUser(evt.teamId, user.id).then(
                            async (contacts) => {
                              ...
                              const emails = Array.from(
                                new Set(
                                  Array.from(
                                    await Promise.all([
                                      ...contacts.map((c) =>
                                        loadContactEmailAddresses(c.id)
                                      ),
                                      ...members.map((m) =>
                                        loadMemberEmailAddresses(m.id)
                                      ),
                                    ])
                                  )
                                    .map((addrs) =>
                                      addrs.map((em) => em.email).join(", ")
                                    )
                                    .filter(Boolean)
                                )
                              ).join(", ");
                                ...
                                new Set(
                                  Array.from(
                                    await Promise.all([
                                      ...contacts.map(
                                        async (c) =>
                                          await loadContactPhoneNumbers(c.id)
                                      ),
                                      ...members.map(
                                        async (m) =>
                                          await loadMemberPhoneNumbers(m.id)
                                      ),
                                    ])
                                  )
                                    .map((phones) =>
              ...
              <div>
                <p>
                  Click this to get your event & contact information from
                  TeamSnap instead of entering it manually:
                </p>
                <div>
                  <button
